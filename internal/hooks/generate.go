...
// GenerateConfig holds the project-specific settings baked into the generated skill.
type GenerateConfig struct {
	TargetDir   string
	ProjectName string
	DocRoots    []string
	// AnalyzeExclude is forwarded as informational context.
	AnalyzeExclude []string
	// HooksFile is the path to the existing hooks.yaml (from src_hooks_file).
	HooksFile string
}
...
1. **Read existing hooks.yaml** (if ` + "`{{.HooksFile}}`" + ` exists) — preserve all existing rules.
...
3. **Run ` + "`aicfg analyze {{.TargetDir}}`" + `** (if not already run in Step 1) for hubs, hotspots, clusters.
...
1. Start with all **existing rules** from ` + "`{{.HooksFile}}`" + ` (preserved exactly).
...
var generateTemplate = template.Must(template.New("skill").Funcs(template.FuncMap{
	"formatList": generateFormatList,
}).Parse(skillTemplate))

// GenerateSkill returns the content of a project-specific hooks-generator SKILL.md.
func GenerateSkill(cfg GenerateConfig) string {
	var buf bytes.Buffer
	if err := generateTemplate.Execute(&buf, cfg); err != nil {
		panic("hooksgen: executing skill template: " + err.Error())
	}
	return buf.String()
}

func generateFormatList(items []string) string {
	if len(items) == 0 {
		return "none"
	}
