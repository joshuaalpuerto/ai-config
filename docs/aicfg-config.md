...
| `doc_audit` | — | Configuration for the `aicfg docaudit` skill generator. |
...
Controls the skill generated by `aicfg docaudit`. All fields are optional.
...
| Field | Required | Description |
|-------|----------|-------------|
| `paths` | — | The full documentation corpus the audit reads. May include AI-assistance configuration paths (`CLAUDE.md`, `AGENTS.md`, `.claude/`, `.github/instructions/`, etc.) to have them treated as in-scope docs. Overrides the default auto-detection of `docs/` and `README.md`. |
